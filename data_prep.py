...
def word_tokenize(txt):
    try:
        data = {'txt': txt}
        respond = utils.call_api(
            data, "http://localhost:4567/tokenize", "POST")
...
if __name__ == '__main__':
    tp = TextPreprocess()
    df = pd.read_csv("../topic_detection_train.v1.0.csv", sep='\t', header=None,
                     names=['label', 'text'], dtype={'label': str, 'text': str})

    df['text'] = [tp.preprocess(val) for val in df['text']]
